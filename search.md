### Goal
Design a robust, fast, user‑friendly text search that finds Courses (and later Teachers, Lesson Formats, tags, etc.) with broad matching, good ranking, and clean UI integration.

### Quick landscape: three viable approaches
- Option A — Enhanced LIKE + Trigram (fastest MVP):
  - Use PostgreSQL `pg_trgm` indexes to accelerate `ILIKE`/similarity across `name`, `summary`, `description`, `slug`.
  - Pros: extremely simple, works with typos/partials. Cons: no stemming, limited ranking control.
- Option B — PostgreSQL Full‑Text Search (balanced, no extra infra):
  - Weighted `tsvector` built from multiple fields, Dutch/English analyzers, `websearch_to_tsquery(q)` for Google‑like syntax, `ts_rank_cd` for ranking, `ts_headline` for highlighting.
  - Combine with `pg_trgm` for typo tolerance and prefix/substring matches.
  - Pros: great quality, tunable ranking, low ops. Cons: some SQL complexity.
- Option C — External engine (Meilisearch/Typesense/Elasticsearch):
  - Great for typo tolerance, facets, aggregations, synonyms, semantic-ish ranking.
  - Pros: best UX at scale. Cons: new infra, sync pipeline, operational overhead.

If you want the best “value for effort” right now, pick Option B and layer in `pg_trgm` for typos.

---

### Option B in detail (recommended)
